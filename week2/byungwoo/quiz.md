...
```
정답 SNI 사용
SNI를 사용하면 여러 HTTPS 어플리케이션을 노출할 수 있습니다.
https://aws.amazon.com/ko/blogs/korea/application-load-balancers-now-support-multiple-tls-certificates-with-smart-selection-using-sni/
```
...
ASG는 인스턴스를 제어하지만 어플리케이션은 제어하지 않습니다.
```
...
```
정답 c. 
EC2 인스턴스에 대한 평균적인 지표를 기준으로 사용하는 정책은 대상 추적정책입니다.
...
```
정답 b
EC2 Auto Scaling 라이프사이클 훅을 사용하면 인스턴스가 종료되기 전에 특정 작업을 추가할 수 있습니다.
